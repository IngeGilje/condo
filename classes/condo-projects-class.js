class Projects extends Condos {
  ...
  getSelectedProjectId(className) {
    ...
    if (isClassDefined(className)) {
      ...
      projectId = (projectId === 0) ? this.arrayProjects.at(-1)?.projectId ?? 0 : projectId;
    } else {

      // Get last id in last object in project array
      projectId = (this.arrayProjects.length > 0) ? this.arrayProjects.at(-1)?.projectId ?? 0 : 0;
    }
  ...
  async getHighestProjectId(condominiumId) {
    const URL = (this.serverStatus === 1)
      ? '/api/projects'
      : 'http://localhost:3000/projects';
    try {
