...
async function events() {
  ...
  document.addEventListener('click', async (event) => {
    if (event.target.classList.contains('delete')) {
      ...
      // Show filter
      const newsId = objNews.arrayNews.at(-1).newsId;
      menuNumber = showFilter(menuNumber, newsId);
      menuNumber = showNews(menuNumber, newsId);
    };
  });
  ...
  document.addEventListener('click', async (event) => {
    if (event.target.classList.contains('cancel')) {
      ...
      // Reload news table
      await objNews.loadNewsTable(condominiumId, objNews.nineNine);
...
function showNews(menuNumber, newsId) {
  ...
  // author
  const author = (rowNumberNews === -1)
    ? Number(objNews.userId)
    : Number(objNews.arrayNews[rowNumberNews].author);
  html += objUser.showSelectedUsers('author', 'width:175px;', author, 'Velg forfatter', '', enableChanges);
  html += "</tr>";
...
async function updateCondoRow(newsId) {
  ...
  const validAuthor = objNews.validateNumber('author', author, 1, objNews.nineNine, objNews, '', 'Ugyldig forfatter');
  ...
  if (validNewsId && validTitle && validDate && validAuthor && validImage) {
    ...
    if (rowNumberNews !== -1) {
      ...
      // update the news row
      await objNews.updateNewsTable(newsId, objNews.user, date, author, title, content, image);
      await objNews.loadNewsTable(objNews.condominiumId, newsId);
    } else {

      // Insert the news row in news table
      await objNews.insertNewsTable(objNews.condominiumId, objNews.user, date, author, title, content, image);
      await objNews.loadNewsTable(objNews.condominiumId, newsId);
