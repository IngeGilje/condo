...
const objCommonCostCondo = new AnnualAccount('annualaccount');

const enableChanges = (objCommonCostCondo.securityLevel > 5);
...
async function main() {
  ...
  if (await objUser.checkServer()) {
    ...
    // Validate LogIn
    if ((objCommonCostCondo.condominiumId === 0) || (objCommonCostCondo.user === null)) {
      ...
      // Show horizonal menu
      let html = objCommonCostCondo.showHorizontalMenu();
      document.querySelector('.horizontalMenu').innerHTML = html;

      const resident = 'Y';
      await objUser.loadUsersTable(objCommonCostCondo.condominiumId, resident, objCommonCostCondo.nineNine);
      await objCondominium.loadCondominiumsTable();
      await objCondo.loadCondoTable(objCommonCostCondo.condominiumId, objCommonCostCondo.nineNine);
      await objCommonCost.loadCommonCostsTable(objCommonCostCondo.condominiumId);
      await objBudget.loadBudgetsTable(objCommonCostCondo.condominiumId, objCommonCostCondo.nineNine, objCommonCostCondo.nineNine);
      await objBankAccount.loadBankAccountsTable(objCommonCostCondo.condominiumId, objCommonCostCondo.nineNine);
      const fixedCost = 'A';
      await objAccount.loadAccountsTable(objCommonCostCondo.condominiumId, fixedCost);
      ...
      const rowNumberCondominium = objCondominium.arrayCondominiums.findIndex(condominium => condominium.condominiumId === objCommonCostCondo.condominiumId);
      if (rowNumberCondominium !== -1) {

        // Show annual accounts
        //const orderBy = 'condoId ASC, date DESC, income ASC';
        //await objBankAccountTransaction.loadBankAccountTransactionsTable(orderBy, objCommonCostCondo.condominiumId, deleted, objCommonCostCondo.nineNine, objCommonCostCondo.nineNine, 0, fromDate, toDate);
...
async function events() {
  ...
  document.addEventListener('click', async (event) => {
    if (event.target.classList.contains('logOut')) {
      ...
      let url = (objCommonCostCondo.serverStatus === 1)
        ? 'http://ingegilje.no/'
        : 'http://localhost/';
...
function showHeader() {
  ...
  // Start table
  let html = objCommonCostCondo.initializeTable(columnWidths);

  // start table body
  html += objCommonCostCondo.startTableBody();

  // show main header
  html += objCommonCostCondo.showTableHeaderLogOut('', '', '', 'Felleskostnader', '',);
  html += "</tr>";

  // end table body
  html += objCommonCostCondo.endTableBody();

  // The end of the table
  html += objCommonCostCondo.endTable();
...
function showFilter(menuNumber, year) {
  ...
  // Header filter (<tr></tr>)
  menuNumber++;
  html += objCommonCostCondo.showTableHeaderMenu(menuNumber, objCommonCostCondo.accountMenu, '', '', '', 'År', '', '');
  ...
  menuNumber++;
  html += objCommonCostCondo.insertTableRow('', menuNumber, objCommonCostCondo.accountMenu, '','');

  // Year (<td></td>)
  html += objCommonCostCondo.showSelectedNumbers('filterYear', 'width:175px;', 2020, 2030, year, true);
  html += "<td></td><td></td></tr>";
...
function showCommonCostCondo(menuNumber) {
  ...
  // Table header (<tr></tr>)
  menuNumber++;
  html += objCommonCostCondo.showTableHeaderMenu(menuNumber, objCommonCostCondo.accountMenu, '#e0f0e0', 'Leilighet', 'Areal', 'Fast beløp', 'Per måned', 'Årlig');
  ...
  objCondo.arrayCondo.forEach((condo) => {
    ...
    html += objCommonCostCondo.inputTableColumn(className, '', condo.name, 45, false);

    // Square meters
    let squareMeters = formatOreToKroner(condo.squareMeters);
    className = `squareMeters${condo.condoId}`;
    html += objCommonCostCondo.inputTableColumn(className, '', squareMeters, 11, false);

    // fixed cost per month per condo
    fixedCostCondoMonth = formatOreToKroner(fixedCostCondoMonth);
    className = `fixedCostCondoMonth${condo.condoId}`;
    html += objCommonCostCondo.inputTableColumn(className, '', fixedCostCondoMonth, 10, false);
    ...
    html += objCommonCostCondo.inputTableColumn(className, '', commonCostsMonth, 11, enableChanges);
    ...
    html += objCommonCostCondo.inputTableColumn(className, '', commonCostsCondoYear, 10, false);
  ...
  menuNumber++;
  html += objCommonCostCondo.insertTableRow('', menuNumber, objCommonCostCondo.accountMenu, 'Sum', totalSquareMeters, totalFixedCostsCondoYear, totalCommonCostsCondoMonth, totalCommonCostsCondoYear);
  html += "</tr>";
