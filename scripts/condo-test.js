...
function showBudget() {
  ...
  objBudgets.arrayBudgets.forEach((budget) => {
    ...
    // from date
    let fromDateHTML = '';
    if (lineNumber === 1) fromDateHTML = objAccounts.showInputHTML('input-fromDate', 'Fra dato', 10, 'mm.dd.åååå');
    ...
    // Row
    //html += HTMLTableRow('<div><label class="one-line">Fra dato</label><input type="text" class="icon-input one-line" maxlength="10" placeholder="mm.dd.åååå"></div>', lineNumber, accountName, amount);
   html += HTMLTableRow(fromDateHTML, lineNumber, accountName, amount);
